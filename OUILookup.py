...
def obtener_tabla_arp(): # Ejecuta el comando arp para obtener las direcciones MAC desde la tabla ARP y devolverlas en una lista.
    #Dependiendo del sistema operativo, la función se ejecuta de manera distinta.
    if os.name == "posix":
        if sys.platform == "darwin":  # Detectar macOS
            salida_arp = os.popen('arp -a').read()  # Comando arp para macOS
        else:  # Linux
            salida_arp = os.popen('arp -n').read()
    elif os.name == "nt":  # Para dispositivos Windows
        salida_arp = os.popen('arp -a').read()
    else: # En caso de no encontrar un sistema operativo compatible se entrega el siguiente mensaje
        return "Sistema operativo no soportado."
    
    mac_addresses = [] # Se crea una lista vacia, donde se almacenarán las direcciones MAC que se encuentren
    # en la tabla ARP. Cada línea de la salida ARP puede contener una dirección MAC, y dependiendo del sistema operativo, 
    # las direcciones MAC pueden aparecer en diferentes columnas.
    
    for linea in salida_arp.splitlines():
        if os.name == "posix":
            if sys.platform == "darwin":
                # En macOS, las MAC suelen estar después de la palabra "at"
                partes = linea.split()
                if "at" in partes:
                    index_mac = partes.index("at") + 1 
                    mac_address = partes[index_mac]
                    if ':' in mac_address:
                        mac_addresses.append(mac_address)
            else:
                # En Linux, las MAC suelen estar en la columna 3
                partes = linea.split()
                if len(partes) >= 3 and ':' in partes[2]:
                    mac_addresses.append(partes[2])
        elif os.name == "nt":
            # En Windows, las MAC suelen estar en la columna 2
            partes = linea.split()
            if len(partes) >= 2 and '-' in partes[1]:
                mac_addresses.append(partes[1].replace('-', ':'))  # Cambiamos '-' por ':' para estandarizar
